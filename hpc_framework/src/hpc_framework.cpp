      ...
      switch (event.type()) {
        ...
        case Event::FAILURE: {
          ...
          if (event.failure().has_executor_id()) {
            // Executor failed.
            cout << "Executor '" << event.failure().executor_id().value()
                 << "' terminated";

            if (event.failure().has_agent_id()) {
              cout << " on Agent '" << event.failure().agent_id().value()
                   << "'";
            }
  ...
  void resourceOffers(const vector<Offer>& offers) {
    for (const Offer& offer : offers) {
      ...
      while (tasksLaunched < totalTasks &&
        ...
        command_stream << "sbatch_test.script task" << taskId << ".test";
...
int main(int argc, char** argv) {
  ...
  if (value.isSome()) {
    uri = path::join(value.get(), "slurm-executor");
  } else {
  ...
  string role;
  flags.add(&role, "role", "Role to use when registering", "*");

  Option<string> master;
  flags.add(&master, "master", "ip:port of master to connect");
  ...
  process::Owned<SlurmScheduler> scheduler(
      new SlurmScheduler(framework, executor, master.get()));
